...
**Time/WiFi**: `setupTime()` uses WiFiManager's captive portal (AP `WorkTimer-Setup`); no hardcoded credentials. After NTP sync the radio is turned off to save power, so WiFi only runs at boot.

**Rendering**: full-screen 16-bit `TFT_eSprite` back-buffer (`render()` clears → draws → `pushSprite`), redrawn ~30 fps. All screens share `drawCard()` (rounded slate card, transparent digits, optional flip "seam", colored border). `drawClock()` = two flip cards (HH/MM) + blinking colon + seconds bar; `drawTimer()` = one 7-seg time card with state-colored border, plus pomodoro chip / progress bar / cycle dots. Sprite needs PSRAM (`-DBOARD_HAS_PSRAM`); check serial for `[SPR] alloc fail`.
